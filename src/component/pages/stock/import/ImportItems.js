...
export default function ImportItems() {
    ...
    const onSubmitImport = async () => {
        if (items.length == 0) {
            showMessage("กรุณาเลือกสินค้านำเข้า", "error");
        }
        const payload = [];
        items.map((item) => {
            if (item.boms.length === 0) {
                ...
                const errorMessage = `คุณไม่สามารถนำเข้าสินค้า ที่มี BOM ได้ กรุณานำเข้าวัตถุดิบแทน`;
                showMessage(errorMessage, 'error');
                return;
            }
        })
        ...
        console.log(payload);
        const res = await stockService.importStock({ lines: payload });
        if (!res) {
            return;
