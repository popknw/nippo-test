...
export default function AdjustmentItems() {
    ...
    const onSubmitImport = async () => {
        ...
        items.map((item) => {
            if (item.boms.length === 0) {
                ...
                const errorMessage = `คุณไม่สามารถปรับสต๊อก ที่มี BOM ได้ กรุณาปรับสต๊อกวัตถุดิบแทน`;
                showMessage(errorMessage, 'error');
                return;
            }
        })
        ...
        const res = await stockService.adjustmentStock({ lines: payload });
        if (!res) {
            return;
